...
def upload_directory_path(instance, filename):
    ...
    return "shop/{}/{}.{}".format(
        now().strftime("%Y-%m-%d"), uuid.uuid4(), filename.split(".")[-1]
    )
...
class Product(
    model_utils_models.SoftDeletableModel, model_utils_models.TimeStampedModel
):
    STATUS_CHOICES = Choices(
        (0, "enabled", _("enabled")),
        (1, "disabled", _("disabled")),
...
class Order(model_utils_models.SoftDeletableModel, model_utils_models.TimeStampedModel):
    ...
    order_no = models.UUIDField(
        verbose_name=_("order no"), unique=True, default=uuid.uuid4, editable=False
    )
...
class CustomerQuestion(
    model_utils_models.SoftDeletableModel, rakmai_models.AbstractPage
):
    CATEGORY_CHOICES = Choices(
        (0, "registration", _("Registration")),
        (1, "verification", _("Verification")),
...
class NaverOrderProductVoucher(
    model_utils_models.SoftDeletableModel, model_utils_models.TimeStampedModel
):
    order_product = models.ForeignKey(
...
class Cart(model_utils_models.TimeStampedModel):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        verbose_name=_("user"),
        db_index=True,
